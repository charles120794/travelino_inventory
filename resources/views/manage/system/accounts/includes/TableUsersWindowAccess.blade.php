<form method="post" action="{{ route('accounts.route',['path' => $path,'action' => 'update-users-window','id' => encrypt($thisUserAccount->users_id)]) }}" id="form_update_users_window">
	...
	<table class="table table-bordered table-condensed table-hover" id="users_table">
		<thead>
			<tr style="font-size: 12px; white-space: nowrap;" class="bg-gray-light">
				<th class="text-center" style="width: 10%;"> <i class="fa fa-check-square-o" style="cursor: pointer;"></i> </th>
				<th class="text-left" style="width: 45%;"> DESCRIPTION </th>
				<th class="text-left" style="width: 45%;"> PATH </th>
			</tr>
		</thead>
		<tbody id="users_table_body">
			@forelse(collect($allWindow)->sortBy('menu_name') as $key => $value)
