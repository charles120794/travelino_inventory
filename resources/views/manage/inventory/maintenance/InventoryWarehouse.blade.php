...
<section class="content">
    ...
    <div class="box box-solid">
        <div class="box-body">
            <form class="form-horizontal" method="get" id="form-search">
                <div class="form-group">
                    <div class="col-sm-12">
                        <input type="text" class="form-control" name="search" autocomplete="search-warehouse" placeholder="Search Warehouse" value="{{ request()->get('search') }}">
                    </div>
                </div>
            </form>
        </div>
        <div class="box-footer text-right">
            ...
            <button type="button" class="btn btn-primary btn-flat" data-toggle="modal" data-target="#modaladdwarehouse"><i class="fa fa-plus"></i> Create </button>
        </div>
    </div>
    ...
    <div class="box box-primary">
        <div class="box-body no-padding">
            ...
            {{ $warehouse->links('vendor.pagination.m_custom_pagination') }}
        </div>
    </div>
...
<script type="text/javascript">
    ...
    $(function(){
        ...
        $(document).on('click','.add-group-option',function(){
            ...
            var input2 = $('<input type="text" class="form-control" name="option[' + count + '][code]" autocomplete="warehouse-code" maxlength="50" value="' + uniqid + '" required>'
                            +'<input type="hidden" name="option[' + count + '][address]">'
                            +'<input type="hidden" name="option[' + count + '][contact]">');
            var input3 = $('<label>Description</label> <span class="text-red">*</span>');
            var input4 = $('<input type="text" class="form-control" name="option[' + count + '][description]" autocomplete="warehouse-description" maxlength="100" required>');
