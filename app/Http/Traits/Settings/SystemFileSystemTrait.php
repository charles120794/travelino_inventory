...
trait SystemFileSystemTrait
{

    public function file_system_upload_image_content($method, $id = null, $request)
    {
        ...
        if($request->hasfile('images'))
        {
            ...
            $request->session()->flash('success', count($request->images) . ' image(s) success addedd.');

            return back();

        } else {

            $request->session()->flash('failed','Invalid Type of Images');
    ...
    public function file_system_update_image_content($method, $id = null, $request)
    {
        ...
        if($request->input('submit') == 'update') {
            ...
            ->update([
                ...
                'media_updated_by'   => $this->activeUser()->users_id,
